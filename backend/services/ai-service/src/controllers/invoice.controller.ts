...
export class InvoiceIntelligenceController {
  async analyze(req: IAuthenticatedRequest, res: Response) {
    try {
      const analysis = await invoiceAnalysisService.analyze(req.params.invoiceId, req.user!.userId);
  ...
  async getAnalysis(req: IAuthenticatedRequest, res: Response) {
    try {
      ...
      if (scope.isVendor) {
        const invoice = await invoiceAnalysisService.getInvoice(invoiceId);
